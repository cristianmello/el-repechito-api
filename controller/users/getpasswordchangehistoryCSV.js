...
const getPasswordChangeHistoryCSV = async (req, res) => {
    try {
        ...
        if (!logs.length) {
            return res.status(404).json({ message: 'No se encontraron registros de cambio de contraseña' });
        }
        ...
        const mapped = logs.map(log => ({
            ...
            'Fecha de cambio': log.created_at.toLocaleString('es-AR')
        }));
        ...
        const fileName = `historial-cambios-password-${today}.csv`;

        res.header('Content-Type', 'text/csv');
        res.attachment(fileName);
        return res.send(csv);
    } catch (error) {
        console.error('Error al exportar CSV de cambio de contraseña:', error);
        res.status(500).json({ message: 'Error interno del servidor al generar el CSV' });
    }
};

module.exports = getPasswordChangeHistoryCSV;
